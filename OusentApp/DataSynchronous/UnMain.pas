...
uses
  ...
  Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.ExtCtrls, System.ImageList,
...
begin
 ...
 { TrayIcon1.Icons := TImageList.Create(Self);
  ...
  TrayIcon1.BalloonHint := '双击系统图标恢复窗口.';
...
var
  sql: string;
  Fsn, Sn, OutputPower, ExtinctionRatio, EyeMask, ReceiverSensitivity, LOS_Assert, LOS_Deassert, MaxInputPower, FSJKGGL, JSJKGGL, RxPower_Point2, RxPower_Point3, TxCal_Error, RxCal_Error, RxCal_Error2, RxCal_Error3, FinalTestId: string;
begin
   //查找最大的ID值
  ...
  if qry.FieldByName('num').asinteger = 0 then
   psql := 'SELECT  * FROM   MES_CodeWritingUnit  WHERE  ID=638500 '
  else
    psql := 'SELECT top 50 * FROM   MES_CodeWritingUnit  WHERE  ID>' + qry.FieldByName('num').AsString
         +' order by id asc ';
  ...
  begin
    ...
    begin
      ...
      if ExtinctionRatio = '' then
        ExtinctionRatio := '0.00';
      ...
      if RxCal_Error2 = '' then
        RxCal_Error2 := '0.00';

      RxCal_Error3 := qry_old2.FieldByName('RxCal_Error3').AsString;
      if RxCal_Error3 = '' then
        RxCal_Error3 := '0.00';


         //生成插入数据的sql语句
       //  sql:=sql+'
    sql := sql + '  insert into Qc_SalesOrderReport(Fsn,Sn,OutputPower,ExtinctionRatio,' + 'EyeMask,ReceiverSensitivity,LOS_Assert,LOS_Deassert ,MaxInputPower,FSJKGGL,JSJKGGL' + ',RxPower_Point2,RxPower_Point3,TxCal_Error,RxCal_Error,RxCal_Error2,RxCal_Error3,FinalTestId)' + ' values(''' + Fsn + ''',''' + Sn + ''',''' + OutputPower + ''', ''' + ExtinctionRatio + ''',' + '''' + EyeMask + ''',''' + ReceiverSensitivity + ''',''' + LOS_Assert + ''',''' + LOS_Deassert + ''',''' + MaxInputPower + ''',' + FSJKGGL + ',' + JSJKGGL + ',''' + RxPower_Point2 + ''',''' + RxPower_Point3 + ''',''' + TxCal_Error + ''',''' + RxCal_Error + ''',''' + RxCal_Error2 + ''',''' + RxCal_Error3 + ''',' + FinalTestId + ')';
